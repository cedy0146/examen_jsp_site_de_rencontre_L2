        ...
        <c:choose>
            <c:when test="${not empty partners}">
                <c:forEach var="partnerId" items="${partners}">
                    <c:set var="partner" value="${partnerDAO != null ? partnerDAO.findById(partnerId) : null}" />
        ...
        <c:choose>
            <c:when test="${not empty messages}">
                <div class="chat-header">
                    <img src="${ctx}/assets/images/default-avatar.png" class="profile-photo-sm" alt="">
                    <div>
                        <strong>${partner != null ? partner.prenom : 'Utilisateur #'}</strong>
                        <p class="text-muted" style="margin:0;font-size:0.85rem;">
                            ${partner != null ? partner.localisation : ''}
                        </p>
                    </div>
                ...
                <div class="chat-messages">
                    <c:forEach var="msg" items="${messages}">
                        <div class="message ${msg.expediteurId == user.id ? 'sent' : 'received'}">
                            <div>${msg.contenu}</div>
                            <div class="message-time">
                                <fmt:formatDate value="${msg.dateEnvoi}" pattern="dd/MM HH:mm"/>
                                <c:if test="${msg.expediteurId == user.id}">
                                    ${msg.lu ? '&#10003;&#10003;' : '&#10003;'}
                                </c:if>
                            </div>
                    </c:forEach>
